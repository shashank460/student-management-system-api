...
export const refreshSchema = z.object({ refreshToken: z.string().min(20).max(4096) }).strict();
...
export const attendanceCreateSchema = z.object({ student: objectId, date: z.coerce.date(), status: z.enum(['present', 'absent', 'late']) }).strict();
export const attendanceUpdateSchema = z.object({ date: z.coerce.date().optional(), status: z.enum(['present', 'absent', 'late']).optional() }).strict().refine((value) => Object.keys(value).length > 0, 'At least one field is required');
...
const subjectSchema = z.object({ name: z.string().trim().min(1).max(100), marks: z.coerce.number().min(0).max(100) }).strict();
export const academicCreateSchema = z.object({ student: objectId, semester: z.coerce.number().int().min(1).max(12), subjects: z.array(subjectSchema).min(1).max(20) }).strict();
export const academicUpdateSchema = z.object({ semester: z.coerce.number().int().min(1).max(12).optional(), subjects: z.array(subjectSchema).min(1).max(20).optional() }).strict().refine((value) => Object.keys(value).length > 0, 'At least one field is required');
